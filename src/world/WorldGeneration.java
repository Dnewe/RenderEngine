...
public class WorldGeneration {
    ...
    private static VoxelArray voxelArray;
    ...
    public static VoxelArray generateWorld() {
        Logger.info("Generating world...");
        voxelArray = new VoxelArray(worldSize);

        //generateGround();
        setVoxel(new VoxelRepresentation(Color.WHITE, 0, 0, 0.5f),128, groundHeight+2, 128);
        generateChicken(135, 5, 135);
        generateSphere(groundVoxel, 90, groundHeight+30, 160, 30);
    ...
    private static void generateCheckerGround() {
        ...
        for (int x= 0 ; x< worldSize[0]; x++) {
            for (int z= 0 ; z< worldSize[2]; z++) {
                if ((x+z)%2 ==0) {
                    setVoxel(groundVoxel, x, groundHeight, z);
                } else {
                    setVoxel(groundVoxel2, x, groundHeight, z);
                }
            }
        }
    }
    ...
    private static void generateSphere(VoxelRepresentation voxel, int centerX, int centerY, int centerZ, int radius) {
        Logger.info("Generating sphere...");
        for (int x=-radius; x<radius; x++) {
            for (int y=-radius; y<radius; y++) {
                for (int z=-radius; z<radius; z++) {
                    if (x*x + y*y + z*z < radius*radius) {
                        setVoxel(voxel, centerX+x, centerY+y, centerZ+z);
                    }
                }
            }
        }
    ...
    private static void setVoxel(VoxelRepresentation voxel, int x, int y, int z) {
        voxelArray.addVoxel(voxel, x, y, z);
    }
